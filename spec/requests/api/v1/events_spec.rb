...
RSpec.describe "Api::V1::Events" do
  describe "GET /index" do
    it "適切な数のイベントが返されること" do
      ...
      get api_v1_events_path, params: { page: 2 }, headers: { "X-Requested-With": "XMLHttpRequest" } # Ajaxリクエストを模倣するためにヘッダーを追加
  ...
  describe "GET /show" do
    it "適切にイベント詳細が返されること" do
      event = create(:event)
      get api_v1_event_path(event), params: { id: event.id }, headers: { "X-Requested-With": "XMLHttpRequest" }
      expect(response).to have_http_status(:ok)
      json_response = res_body # レスポンスJSONをハッシュで返す
