...
module UserAuth
  class AccessToken
    ...
    private
      ...
      def claims
        claim_data = {}
        claim_data[:exp] = token_expiration
        claim_data[user_claim] = @user_id if verify_user_id?
        claim_data[:iss] = token_issuer if verify_issuer?
        claim_data[:aud] = token_audience if verify_audience?
        claim_data
      end
